...
import { getUserPayload } from "../middlewares/jwt/getUserPayload";
import UserService from "../services/UserService";
import { validateCreateGallery } from "../middlewares/validators/CreateGalleryValidator";
import formidable from "formidable";
import { UserEntity } from "../database/entities/UserEntity";
import { UpdateGalleryRules } from "../middlewares/validators/UpdateGalleryValidator";
...
export class GalleryController {
  ...
  public create = async (req: Request, res: Response) => {
    ...
    form.parse(req, async (errForm: any, fields: any, files: any) => {
       ...
       */
      const { id } = getUserPayload(req, res);
      const user: Promise<UserEntity> =
        this.userService.getSingleUserDetailsFromId(Number(id));

      let oldPath: string = files.image.path;
      const baseDirectory: string = path.join(
        __dirname,
      ...
      return await this.writeFileOnCreate(
        directory,
        rawData,
        res,
        user,
  ...
  public writeFileOnCreate = async (
    directory: string,
    rawData: any,
    res: Response,
  ...
  public update = async (req: Request, res: Response): Promise<any> => {
    ...
    form.parse(req, async (errForm: any, fields: any, files: any) => {
      ...
      const directory: string = baseDirectory + "/" + newName;
      let rawData = fs.readFileSync(oldPath);
      return await this.writeFileOnEdit(
        directory,
        rawData,
        res,
        newName,
  ...
  public updateWithoutNewFile = async (
    req: Request,
    res: Response,
    fields: any
  ): Promise<Response> => {
  ...
  public deleteFileAfterUpdateOrDelete = async (id: number) => {
    const singleGallery: GalleryEntity | undefined =
      await this.galleryContent.getSingleGallery(id);
    const fileName: string | undefined = singleGallery?.image;
    const pathToFile: string = path.join(
      __dirname,
      `../public/uploads/gallery/${fileName}`
    );
